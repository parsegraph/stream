...
const Line = ({line, dir})=>{
  ...
  return <parsegraph dir={dir} type="u" pull="f">
    {words.reverse().reduce((cellChildren, word)=>(
      <parsegraph dir={dir} type="b" label={word}>
        {cellChildren}
      </parsegraph>
    ), null)}
  </parsegraph>
...
const Page = ({name, content})=>{
  ...
  return <parsegraph type='b' label={name}>
    {lines.reverse().reduce((cellChildren, line)=>(
      <parsegraph dir='d' type="u" pull='f'>
        <Line dir='f' line={line}/>
        {cellChildren}
      </parsegraph>
    ), null)}
  </parsegraph>
  } catch (ex) {
    return <parsegraph type='b' label={ex}/>
  }
}
