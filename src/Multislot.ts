...
export default class Multislot {
  ...
  constructor(room:Room, rowSize:number, columnSize:number, color:Color, subtype:MultislotSubtype)
  {
      this._room = room;
      this._plots = [];
      this._id = null;

      var car = new BlockCaret('b');
      this._root = car.node();
      ...
      var multislotActions = new ActionCarousel(room.carousel());
  ...
  build(car: BlockCaret, subtype:MultislotSubtype) {
      const spawnPlot = ()=>{
          ...
          var plot = new MultislotPlot(this, index);
          this._plots.push(plot);
          return plot;
      };

      const cs = copyStyle('u');
      cs.backgroundColor = new Color(.8);
      cs.borderColor = new Color(.6);
...
spawnItem(room:Room, value:any, children:any[], id:number) {
    ...
    var color = new Color(params[3]/255, params[4]/255, params[5]/255);
    var multislot = new Multislot(room, rowSize, columnSize, color, subtype);
    multislot.setId(id);
