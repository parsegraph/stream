...
class ParsegraphNode {
  ...
  setClickListener(cb) {
    this.server().send("setClickListener", this.id(), this.server().addCallback(cb));
  }

  callback(cb) {
    this.setClickListener(cb)
  }
  ...
  ensureNeighbor(dir) {
    if (!this._neighbors[dir]) {
      this._neighbors[dir] = {owner: this, direction: dir};
    }
  ...
  textEdit(val, cb) {
    this.server().send("textEdit", this.id(), val, this.server().addCallback(cb));
  }

  textSplice(val, offset, len, subPath) {
    this.server().send("textSplice", this.id(), val, offset, len, subPath);
  }
  ...
  setNodeAlignmentMode(inDirection, newAlignmentMode) {
  ...
  parentNode() {
    return this.nodeAt(this.parentDirection())
  }
  ...
  disconnectNode(inDirection) {
    ...
    this.server().send("disconnectNode", this.id(), inDirection)
